...
public class PoiSheet implements Sheet {
    ...
    PoiSheet(final org.apache.poi.ss.usermodel.Sheet delegate) {
        ...
        numberOfRows = this.delegate.getLastRowNum() + 1;
        name = this.delegate.getSheetName();
    }
    ...
    public String[] getRow(final int rowNumber) {
        final Row row = delegate.getRow(rowNumber);
        if (row == null) {
            return null;
        }
    ...
    private FormulaEvaluator getFormulaEvaluator() {
        if (evaluator == null) {
            evaluator = delegate.getWorkbook().getCreationHelper().createFormulaEvaluator();
        }
        return evaluator;
    }
    ...
    public int getNumberOfColumns() {
        if (numberOfColumns < 0) {
            numberOfColumns = delegate.getRow(0).getLastCellNum();
        }
